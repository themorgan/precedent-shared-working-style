...
| [`practices/`](practices/) | This set's practices, one file each: [`audience-register.md`](practices/audience-register.md), [`nonblocking-questions.md`](practices/nonblocking-questions.md), [`small-calls.md`](practices/small-calls.md) and [`quiet-checks.md`](practices/quiet-checks.md). All but `quiet-checks.md` are `tier: resident`, so they load on every turn in a project that declares this set. (`example-starter.md`, the placeholder this set was bootstrapped with, was deleted once these landed — which is what its own text asked for.) |
